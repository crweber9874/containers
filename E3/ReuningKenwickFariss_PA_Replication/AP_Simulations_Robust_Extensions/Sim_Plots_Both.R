...
##estimating both degrees of freedom#######
##and sigma################################
###########################################
...
df <- df[df$type=="rob_all"| df$type=="robust",]
...
#### Figure 13 ######
pdf("plots/Figure_13B.pdf", height=12, width=12)
...
for(kk in 1:length(innov)){
  ...
  for(jj in 1:length(prob.shock)){
    ...
    tmp.df <- df[df$prob.shock==prob.shock[jj] & df$innov==innov[kk] & df$type=="rob_all",]
...
mtext(expression("" %<-% "Estimate Sigma                 Estimate Both" %->% ""), 3, cex=2.5)
...
#### Figure 13 ######
pdf("plots/Figure_13A.pdf", height=12, width=12)
par(mar=c(4, 9, 4, 1), cex=1.75)
...
pdf("plots/Figure_13C.pdf", height=12, width=12)
par(mar=c(4, 9, 4, 1), cex=1.75)
...
#### Figure 13 ######
pdf("plots/Figure_13D.pdf", height=12, width=12)
par(mar=c(4, 9, 4, 1), cex=1.75)
plot.new()
...
#### Figure 13 ######
pdf("plots/Figure_13E.pdf", height=12, width=12)
par(mar=c(4, 9, 4, 1), cex=1.75)
...
table(df$prob.shock[df$type=="rob_all"], df$innov[df$type=="rob_all"])
