...
mod.rob <- stan(file = "MAIN_MQ_Replication/MQ_Robust.stan", cores=4, chains=4, iter=10000, thin=10, seed=1355208830, 
...
save(theta.rob, up.rob, lo.rob, file="MAIN_MQ_Replication/rob_theta.RData")

###### Comment out below to not save a ~5 gig file ####
save.image("MAIN_MQ_Replication/robust_all.RData")
...
library(loo)
ll <- extract_log_lik(mod.rob, merge_chains=F)
waic.rob <- waic(ll)

save(waic.rob, file = "MAIN_MQ_Replication/rob_waic.RData")
save(rob.pred, file = "MAIN_MQ_Replication/rob_pred.RData")
...
####### Figure 23 ########
pdf("plots/Figure_23B.pdf", height=6, width=6)
