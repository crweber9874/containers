###########################################
##Generates the robust estimates###########
...
free_sigma <- rep(1, length(theta_sd))
free_sigma[theta_past==0] <- 0
free_sigma[theta_sd==.0001] <- 0
...
mod.rob <- stan(file = "AP_MQ_Robustness/MQ_Robust_Free.stan", cores=4, chains=4, iter=2000, thin=2, seed=1101866165,
                open_progress = F, control=list("max_treedepth"=15, "adapt_delta"=.99))

inits_check <- get_inits(mod.rob)
plot(inits_check[[1]]$thetas)

out.rob <- extract(mod.rob)

theta.rob <- apply(out.rob$thetas, 2, mean)
#sd.rob <- apply(out.rob$thetas, 2, sd)
up.rob <- apply(out.rob$thetas, 2, quantile, 0.975)
lo.rob <- apply(out.rob$thetas, 2, quantile, 0.025)

save(theta.rob, up.rob, lo.rob, file="AP_MQ_Robustness/rob_theta_Free.RData")

###### Comment out below to not save a ~5 gig file ####
save.image("AP_MQ_Robustness/robust_all_Free.RData")

tot <- nrow(out.rob$thetas)


rob.pred <- out.rob$pred

library(loo)
ll <- out.rob$log_lik
waic.rob <- waic(ll)

save(waic.rob, file = "AP_MQ_Robustness/rob_waic_Free.RData")
save(rob.pred, file = "AP_MQ_Robustness/rob_pred_Free.RData")
...
####### Figure 23 ########
pdf("plots/Figure_23F.pdf", height=6, width=6)
