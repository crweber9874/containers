###########################################
##Estimates 1 fold of K-fold CV############
##Need to have mltools installed###########
###########################################
###########################################
...
# setwd("~/work/MQ_CV") # SETWD TO MAIN FOLDER NOT SUBFOLDER

library(rstan)
library(mltools)

load("AP_K_Fold/MQ_K_Fold/mqData2015.Rda")
...
part.n <- 10
ids <- folds(justice.term.id, part.n, stratified=T, seed=1)
# ll <- NULL
ii <- ii.pass 

# for(ii in 1:part.n){
  
  test_items_ids <- items_ids[ii==ids]
  train_items_ids <- items_ids[ii!=ids]
  
  test_cont_year_ids <- cont_year_ids[ii==ids]
  train_cont_year_ids <- cont_year_ids[ii!=ids]
  
  test_items <- items[ii==ids]
  train_items <- items[ii!=ids]
  
  test_n_complete <- length(test_items)
  train_n_complete <- length(train_items)
  
  mod.rob <- stan(file = "AP_K_Fold/MQ_K_Fold/MQ_Robust_CV.stan",cores=6, chains=6, iter=8000, thin=10,
                  open_progress = F, control=list("max_treedepth"=15, "adapt_delta"=.99))
  
  
  
  ll <- extract_log_lik(mod.rob)
#   ll <- cbind(ll, tmp.ll)
# }

save(ll, file=paste("AP_K_Fold/MQ_K_Fold/MQ_Rob_CV_", ii, ".RData", sep=""))
q()
