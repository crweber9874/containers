...
mod.stat <- stan(file="MAIN_UDS_Replication/UDS_Static.stan",
            data=stan.data, seed=570175513,
            iter=10000, chains=4, cores=4, thin=5)

######### Comment out to stop creation of 3 gig file ########
save.image("MAIN_UDS_Replication/Pemp_Static.RData")
# load("Pemp_rep.RData")

out <- extract(mod.stat)
df <- df[order(df$count.year.id),]
df$stat.estimates <- apply(out$thetas, 2, median)
df$stat.up <- apply(out$thetas, 2, quantile, 0.975) 
df$stat.lo <- apply(out$thetas, 2, quantile, 0.025) 

write.csv(df, "MAIN_UDS_Replication/stat_est.csv", row.names = F)
...
write.csv(pp.out, "MAIN_UDS_Replication/pp_stat.csv", row.names = F)


source("MAIN_UDS_Replication/UDS_utils.R")


stat.2 <- post_pred_corrs(year_ids=stan.data$years_count_2, stan.data$items_2, draws=out$y_hat_2)
stat.3 <- post_pred_corrs(year_ids=stan.data$years_count_3, stan.data$items_3, draws=out$y_hat_3)
stat.4 <- post_pred_corrs(year_ids=stan.data$years_count_4, stan.data$items_4, draws=out$y_hat_4)
stat.7 <- post_pred_corrs(year_ids=stan.data$years_count_7, stan.data$items_7, draws=out$y_hat_7)
stat.8v <- post_pred_corrs(year_ids=stan.data$years_count_8v, stan.data$items_8v, draws=out$y_hat_8v)
stat.13 <- post_pred_corrs(year_ids=stan.data$years_count_13, stan.data$items_13, draws=out$y_hat_13)
stat.21 <- post_pred_corrs(year_ids=stan.data$years_count_21, stan.data$items_21, draws=out$y_hat_21)

pred_post_stat_est <- matrix(NA, nrow=8000, ncol=10)

pred_post_stat_est[,1] <- apply(out$y_hat_2, 1, function(x) cor(x, stan.data$items_2))
pred_post_stat_est[,2] <- apply(out$y_hat_3, 1, function(x) cor(x, stan.data$items_3))
pred_post_stat_est[,3] <- apply(out$y_hat_4, 1, function(x) cor(x, stan.data$items_4))
pred_post_stat_est[,4] <- apply(out$y_hat_7, 1, function(x) cor(x, stan.data$items_7))
pred_post_stat_est[,5] <- apply(out$y_hat_8h, 1, function(x) cor(x, stan.data$items_8h))
pred_post_stat_est[,6] <- apply(out$y_hat_8v, 1, function(x) cor(x, stan.data$items_8v))
pred_post_stat_est[,7] <- apply(out$y_hat_10, 1, function(x) cor(x, stan.data$items_10))
pred_post_stat_est[,8] <- apply(out$y_hat_11, 1, function(x) cor(x, stan.data$items_11))
pred_post_stat_est[,9] <- apply(out$y_hat_13, 1, function(x) cor(x, stan.data$items_13))
pred_post_stat_est[,10] <- apply(out$y_hat_21, 1, function(x) cor(x, stan.data$items_21))
...
mcmc <- As.mcmc.list(mod.stat, pars=params.use)
...
save(gd, file="MAIN_UDS_Replication/gd_stat.RData")

rm(mcmc)
sapply(1:10, gc)
static.waic <- waic(ll)

save(pred_post_stat_est, static.waic, 
     stat.2, stat.3, stat.4, 
     stat.7, stat.8v, stat.13, 
     stat.21, 
     file="MAIN_UDS_Replication/statistics_stat.RData")
