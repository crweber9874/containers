...
write.csv(df, "MAIN_UDS_Replication/dyn_est.csv", row.names = F)
...
write.csv(pp.out, "MAIN_UDS_Replication/pp_dyn.csv", row.names = F)

source("MAIN_UDS_Replication/UDS_utils.R")

dyn.2 <- post_pred_corrs(year_ids=stan.data$years_count_2, stan.data$items_2, draws=out$y_hat_2)
dyn.3 <- post_pred_corrs(year_ids=stan.data$years_count_3, stan.data$items_3, draws=out$y_hat_3)
dyn.4 <- post_pred_corrs(year_ids=stan.data$years_count_4, stan.data$items_4, draws=out$y_hat_4)
dyn.7 <- post_pred_corrs(year_ids=stan.data$years_count_7, stan.data$items_7, draws=out$y_hat_7)
dyn.8v <- post_pred_corrs(year_ids=stan.data$years_count_8v, stan.data$items_8v, draws=out$y_hat_8v)
dyn.13 <- post_pred_corrs(year_ids=stan.data$years_count_13, stan.data$items_13, draws=out$y_hat_13)
dyn.21 <- post_pred_corrs(year_ids=stan.data$years_count_21, stan.data$items_21, draws=out$y_hat_21)


pred_post_dyn_est <- matrix(NA, nrow=8000, ncol=10)

pred_post_dyn_est[,1] <- apply(out$y_hat_2, 1, function(x) cor(x, stan.data$items_2))
pred_post_dyn_est[,2] <- apply(out$y_hat_3, 1, function(x) cor(x, stan.data$items_3))
pred_post_dyn_est[,3] <- apply(out$y_hat_4, 1, function(x) cor(x, stan.data$items_4))
pred_post_dyn_est[,4] <- apply(out$y_hat_7, 1, function(x) cor(x, stan.data$items_7))
pred_post_dyn_est[,5] <- apply(out$y_hat_8h, 1, function(x) cor(x, stan.data$items_8h))
pred_post_dyn_est[,6] <- apply(out$y_hat_8v, 1, function(x) cor(x, stan.data$items_8v))
pred_post_dyn_est[,7] <- apply(out$y_hat_10, 1, function(x) cor(x, stan.data$items_10))
pred_post_dyn_est[,8] <- apply(out$y_hat_11, 1, function(x) cor(x, stan.data$items_11))
pred_post_dyn_est[,9] <- apply(out$y_hat_13, 1, function(x) cor(x, stan.data$items_13))
pred_post_dyn_est[,10] <- apply(out$y_hat_21, 1, function(x) cor(x, stan.data$items_21))
...
mcmc <- As.mcmc.list(mod.dyn, pars=params.use)
...
save(gd, file="MAIN_UDS_Replication/gd_dyn.RData")
...
save(pred_post_dyn_est, dyn.waic, 
     dyn.2, dyn.3, dyn.4, 
     dyn.7, dyn.8v, dyn.13, 
     dyn.21, 
     file="MAIN_UDS_Replication/statistics_dyn.RData")
